...
featureSelection2 <-function(data,labels,vars_selected,mode="mrmr",disease="",subdiseases=c(),maxGenes=ncol(data)){
...
# The matrix iris.rf$importance has the same columns at the beginning before "MeanDecreaseAccuracy" and "MeanDecreaseGini"
# as classes have iris$Species (3).
# So, to access to MeanDecreaseAccuracy we shouldn't always reference rfRanking$importance[,3], but rfRanking$importance[,"MeanDecreaseAccuracy"]
# The older version of featureSelection with randomForest only seems to work correctly on biclass classification.

# KnowSeq function
library(KnowSeq)
load("../data/saved_files/workspace_multiclase.RData")
rfRanking_knowseq <- featureSelection(particion.entrenamiento, labels_train, colnames(particion.entrenamiento),
                              mode = "rf")
...
load("../data/saved_files/workspace_multiclase.RData")
rfRanking_custom <- featureSelection2(particion.entrenamiento, labels_train, colnames(particion.entrenamiento),
                                      mode = "rf")

# Comparación
length(rfRanking_knowseq)
length(rfRanking_custom)

rfRanking_all <- data.frame(rfRanking_knowseq, rfRanking_custom)
rfRanking_all$equal <- rfRanking_knowseq == rfRanking_custom

table(rfRanking_all$equal)