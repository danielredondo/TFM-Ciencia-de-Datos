...
tic("countsToMatrix") # 93 segundos
...
tic("calculateGeneExpressionValues") # 190 segundos
...
tic("batchEffectRemoval") # 594 segundos
...
# Número de genes extraídos: 6836 de 26945 genes
...
# disease association: 22 segundos
tic("svm_da")
...
# El mejor F1-score se obtiene con 3 genes en RF o mRMR. Elegimos RF por ejemplo
...
dataPlot(DEGsMatrix[mejores_genes_svm, ], labels, mode = "genesBoxplot", colours = c("red", "green"), columnasGenesBoxplot = 3)
dev.off()
...
tabla <- results_svm_mrmr$cfMats[[3]]$table # Mismo resultado
...
which(f1 == max(f1), arr.ind = TRUE)
# El mejor F1 es con mRMR con 4 genes
mejores_genes_rf <- mrmrRanking[1:4]
...
dataPlot(DEGsMatrix[mejores_genes_rf, ], labels, mode = "genesBoxplot", colours = c("red", "green"), columnasGenesBoxplot = 2)
dev.off()
...
# Matriz de confusión mejor clasificador
tabla <- results_rf_mrmr$cfMats[[4]]$table
...
which(f1 == max(f1), arr.ind = TRUE)
# El mejor ranking es RF con 3 genes
...
# Buscar cáncer de colon-recto y enfermedades relacionadas
unique(c(enfermedades[grep(pattern = "cancer", ignore.case = T, x = enfermedades)],
         ...
         enfermedades[grep(pattern = "colon",  ignore.case = T, x = enfermedades)],
         enfermedades[grep(pattern = "rectum",  ignore.case = T, x = enfermedades)],
...
matriz <- matriz[, sample(x = 1:ncol(matriz), 100)]
