...
dataPlot <- function(data, labels, colours = c("green", "red"), main = "", ylab = "Expression", xlab = "Samples",
                     xgrid = FALSE, ygrid = FALSE, legend = "", mode="boxplot", toPNG = FALSE, toPDF = FALSE, columnasGenesBoxplot = 3){
  ...
  if(mode == "boxplot"){
    ...
    boxplot(data[,sortedLabels] , col=coloursAux, ylab=ylab , xlab=xlab, main=main, las = 2, names = rep("", ncol(data)))
    ...
    print(ggplot(xx, aes(x=as.factor(Classes),y=Value,fill=as.factor(Classes))) + geom_boxplot() + facet_wrap(~Gen, ncol = columnasGenesBoxplot) 
          + scale_fill_manual(values=colours) + ggtitle(main) + theme_classic() + xlab(xlab) + ylab(ylab) + labs(fill = "Classes"))
    
    if(toPNG){
      cat("Creating PNG...\n")
      ggplot(xx, aes(x=as.factor(Classes),y=Value,fill=as.factor(Classes))) + geom_boxplot() + facet_wrap(~Gen, ncol = columnasGenesBoxplot) +scale_fill_manual(values=colours) + ggtitle(main) + theme_classic() + xlab(xlab) + ylab(ylab) + labs(fill = "Classes")
    ...
    if(toPDF){
      cat("Creating PDF...\n")
      ggplot(xx, aes(x=as.factor(Classes),y=Value,fill=as.factor(Classes))) + geom_boxplot() + facet_wrap(~Gen, ncol = columnasGenesBoxplot) + scale_fill_manual(values=colours) + ggtitle(main) + xlab(xlab) + ylab(ylab) + labs(fill = "Classes")
    ...
    if(toPNG){
      ...
      heatmap.2(t(data), col=redgreen(75), scale="column", RowSideColors=coloursAux,
    ...
    heatmap.2(t(data), col=redgreen(75), scale="none", RowSideColors=coloursAux ,
              key=TRUE, symkey=FALSE, density.info="none", trace="none", cexRow=1,
              cexCol=1, margins=c(6,11),srtCol=45, Rowv = FALSE, dendrogram = "none", Colv = FALSE)
    ...
    if(toPNG){
      ...
      if(!is.matrix(data)){
        ...
        if(legend != ""){
          legend("bottomright", legend=legend, col=colours[2],lwd = 2.5,cex=0.8,lty = "dashed")
        }
